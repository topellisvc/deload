...
export default async function DashboardPage({
  ...
  /** `?day=<training_day_id>` — only meaningful on the initial load now
   * (e.g. a bookmarked/refreshed URL); the Hero's prev/next browse arrows
   * used to set this and rely on a full page reload to resolve it
   * (getActiveProgramContext), but that reran the ENTIRE dashboard's data
   * fetch just to swap which day was displayed. Browsing now happens
   * entirely client-side (DashboardContent) from data this page already
   * fetched — see day-view.ts. Absent for the normal "today" view.
